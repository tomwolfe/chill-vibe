...
def test_budget_tracker_dynamic_pricing_gemini_2_0():
    # Gemini 2.0 Flash: $0.10/1M input, $0.40/1M output
    tracker = BudgetTracker(model_id="gemini-2.0-flash-exp")
...
def test_budget_tracker_dynamic_pricing_gemini_2_0_pro():
    # Gemini 2.0 Pro: $1.25/1M input, $5.00/1M output
    tracker = BudgetTracker(model_id="gemini-2.0-pro-preview")
    
    # 1M prompt tokens -> $1.25
    # 1M candidate tokens -> $5.00
    response = MockResponse(1_000_000, 1_000_000)
    tracker.update_from_response(response)
